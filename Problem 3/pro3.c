...
int main()
{
   FILE *output_file = 0;
   int gray_level=0;
   printf("input gray level power of 2:");
   scanf("%d",&gray_level);
   ...
   for (i = 0; i < biHeight; i++)
   {
      ...
      for (i = 0; i < biHeight; i++)
      {
         ...
         for (j = 0; j < biWidth; j++)
         {
            data2[k] = (data1[k]/(256/gray_level))*(256/gray_level);
            k = k + 1;
         }
      }
   }
   ...
   //
   /* �}�ҷs�ɮ� */
   char arr1[30]="Gray_scale_with-";
   char arr2[30];
   itoa(gray_level, arr2);
   if ((output_file = fopen(strcat(strcat(arr1,arr2),".bmp"), "wb")) == NULL)
   {
