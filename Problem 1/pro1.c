...
int InTriangle(int x, int y, int x1, int y1, int x2, int y2, int x3, int y3)
{
...
int main()
{
   ...
   for (i = 0; i < biHeight; i++)
   {
      ...
      int x2 = centerX + sideLength / 2, x3 = centerX - sideLength / 2 ;
      int  y2 = centerY - height, y3 = centerY - height;
      ...
      for (i = 0; i < biHeight; i++)
      {
         ...
         for (j = 0; j < biWidth; j++)
         {
            if (InTriangle(j, i, x1, y1, x2, y2, x3, y3))
            //算如果j,i到三頂點的組成的三角形面積總和與大正三角型相同
            {
               data2[k] = 255- data1[k];
            }
            else
